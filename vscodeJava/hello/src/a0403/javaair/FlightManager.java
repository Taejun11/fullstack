...
public class FlightManager {
    ...
    String ticketPrint(Map<String, Flight> reservation, String name) {
        int index = -1;
        if (passengers != null) {
    ...
    public void ticketSave() {
        ...
        checkPw(index);
        fc.ticketSavefile(reservation, passengers.get(index).getName());
        } catch (IndexOutOfBoundsException e) {
            System.out.println("해당 고객의 티켓이 없습니다.");
        }
        
    }

    // 항공편 목록을 외부에서 접근 가능한 get
    public static ArrayList<Flight> getFlight() {
        return flights;
    }
    // 예약정보를 외부에서 접근 가능한 get
    public static Map<String, Flight> getReservation(){
        return reservation;
    }
